...
class AclRolePermissionController {
    private final AclRolePermissionService adminRolePermissionService;

    @Operation(summary = "按条件分页查询所有")
    @PostMapping("/v1/page")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<IPage<AclRolePermission>> v1Page(@RequestBody Page<AclRolePermission> page) {
        QueryWrapper<AclRolePermission> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        IPage<AclRolePermission> result = adminRolePermissionService.page(page, queryWrapper);
        return R.ok(result);
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<List<AclRolePermission>> v1ListAll(@RequestBody AclRolePermission o) {
//        o.setDeleted(DeletedConstants.FALSE);
        QueryWrapper<AclRolePermission> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        return R.ok(adminRolePermissionService.list(queryWrapper));
    }

    @Operation(summary = "按ID获取信息")
    @GetMapping("/v1/info/{id}")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<AclRolePermission> v1Info(@PathVariable String id) {
        return R.ok(adminRolePermissionService.getById(id));
    }
    ...
    public R<Void> v1Delete(@PathVariable String id) {
        AclRolePermission o = new AclRolePermission();
        o.setId(id);
//        o.setDeleted(DeletedConstants.TRUE);
        adminRolePermissionService.updateById(o);
        return R.ok();
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Modify(@RequestBody @Validated AclRolePermission o) {
//        o.setDeleted(DeletedConstants.FALSE);
        adminRolePermissionService.updateById(o);
        return R.ok();
    }
    ...
    @PostMapping("/v1/add")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Add(@RequestBody @Validated AclRolePermission o) {
        o.setId(IdUtil.randomUUID());
//        o.setDeleted(DeletedConstants.FALSE);
        adminRolePermissionService.save(o);
        return R.ok();
    }
}