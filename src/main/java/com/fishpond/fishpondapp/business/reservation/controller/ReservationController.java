package com.fishpond.fishpondapp.business.reservation.controller;
...
class ReservationController {
    ...
    public R<IPage<Reservation>> v1Page(@RequestBody Page<Reservation> page) {
        QueryWrapper<Reservation> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        IPage<Reservation> result = reservationService.page(page, queryWrapper);
        return R.ok(result);
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<List<Reservation>> v1ListAll(@RequestBody Reservation o) {
        o.setDeleted(DeletedConstants.FALSE);
        QueryWrapper<Reservation> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        return R.ok(reservationService.list(queryWrapper));
    }

    @Operation(summary = "按ID获取信息")
    @GetMapping("/v1/info/{id}")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Reservation> v1Info(@PathVariable String id) {
    ...
    public R<Void> v1Delete(@PathVariable String id) {
        ...
        reservationService.updateById(o);
        return R.ok();
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Modify(@RequestBody @Validated Reservation o) {
        o.setDeleted(DeletedConstants.FALSE);
        reservationService.updateById(o);
        return R.ok();
    }
    ...
    @PostMapping("/v1/add")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Add(@RequestBody @Validated Reservation o) {
        o.setId(IdUtil.randomUUID());
        o.setDeleted(DeletedConstants.FALSE);
        reservationService.save(o);
        return R.ok();
    }
}