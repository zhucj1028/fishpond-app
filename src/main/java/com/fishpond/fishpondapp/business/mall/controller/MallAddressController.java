...
class MallAddressController {
    ...
    public R<IPage<MallAddress>> v1Page(@RequestBody Page<MallAddress> page) {
        QueryWrapper<MallAddress> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        IPage<MallAddress> result = mallAddressService.page(page, queryWrapper);
        return R.ok(result);
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<List<MallAddress>> v1ListAll(@RequestBody MallAddress o) {
        o.setDeleted(DeletedConstants.FALSE);
        QueryWrapper<MallAddress> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        return R.ok(mallAddressService.list(queryWrapper));
    }

    @Operation(summary = "按ID获取信息")
    @GetMapping("/v1/info/{id}")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<MallAddress> v1Info(@PathVariable String id) {
        return R.ok(mallAddressService.getById(id));
    }
    ...
    public R<Void> v1Delete(@PathVariable String id) {
        MallAddress o = new MallAddress();
        o.setId(id);
        o.setDeleted(DeletedConstants.TRUE);
        mallAddressService.updateById(o);
        return R.ok();
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Modify(@RequestBody @Validated MallAddress o) {
        o.setDeleted(DeletedConstants.FALSE);
        mallAddressService.updateById(o);
        return R.ok();
    }
    ...
    @PostMapping("/v1/add")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Add(@RequestBody @Validated MallAddress o) {
        o.setId(IdUtil.randomUUID());
        o.setDeleted(DeletedConstants.FALSE);
        mallAddressService.save(o);
        return R.ok();
    }
}