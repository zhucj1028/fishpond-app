...
class CommunityPostController {
    ...
    public R<IPage<CommunityPost>> v1Page(@RequestBody Page<CommunityPost> page) {
        QueryWrapper<CommunityPost> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        IPage<CommunityPost> result = communityPostService.page(page, queryWrapper);
        return R.ok(result);
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<List<CommunityPost>> v1ListAll(@RequestBody CommunityPost o) {
        o.setDeleted(DeletedConstants.FALSE);
        QueryWrapper<CommunityPost> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("deleted", DeletedConstants.FALSE);
        return R.ok(communityPostService.list(queryWrapper));
    }

    @Operation(summary = "按ID获取信息")
    @GetMapping("/v1/info/{id}")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<CommunityPost> v1Info(@PathVariable String id) {
        return R.ok(communityPostService.getById(id));
    }
    ...
    public R<Void> v1Delete(@PathVariable String id) {
        CommunityPost o = new CommunityPost();
        o.setId(id);
        o.setDeleted(DeletedConstants.TRUE);
        communityPostService.updateById(o);
        return R.ok();
    }
    ...
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Modify(@RequestBody @Validated CommunityPost o) {
        o.setDeleted(DeletedConstants.FALSE);
        communityPostService.updateById(o);
        return R.ok();
    }
    ...
    @PostMapping("/v1/add")
    @PreAuthorize("@atc.admin")//默认只有管理员能访问该接口。其他设置：@atc.admin、@atc.user、@atc.inside、@atc.hasAuthority('xxx')
    public R<Void> v1Add(@RequestBody @Validated CommunityPost o) {
        o.setId(IdUtil.randomUUID());
        o.setDeleted(DeletedConstants.FALSE);
        communityPostService.save(o);
        return R.ok();
    }
}